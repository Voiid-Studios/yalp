...
import dev.maxi.yalp.api.component.ComponentLifecycle;
...
public final class ComponentManager implements ComponentRegistry {
    ...
    public void register(YALPComponent component) {
        ...
        if (byClass.containsKey(component.getClass())) {
            throw new IllegalArgumentException("Duplicate YALP component class: " + component.getClass().getName());
        }
    ...
    public void disableAll() {
        ...
        for (YALPComponent component : components) {
            String id = normalize(component.getId());
            if (lifecycle.get(id) == ComponentLifecycle.DISABLED) {
                continue;
            }
            try {
                component.onDisable();
